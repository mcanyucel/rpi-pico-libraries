...
bool ds3231_modded_read_time(ds3231_modded_t *device, ds3231_modded_time_t *time)
{
    ...
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_SECONDS, &raw_sec))
        return false;
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_MINUTES, &raw_min))
        return false;
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_HOURS, &raw_hour))
        return false;
...
bool ds3231_modded_read_date(ds3231_modded_t *device, ds3231_modded_date_t *date)
{
    ...
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_WEEKDAY, &raw_weekday))
        return false;
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_DAY, &raw_day))
        return false;
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_MONTH, &raw_month))
        return false;
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_YEAR, &raw_year))
        return false;
...
{
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_SECONDS, ds3231_modded_dec_to_bcd(time->seconds)))
        return false;
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_MINUTES, ds3231_modded_dec_to_bcd(time->minutes)))
        return false;
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_HOURS, ds3231_modded_dec_to_bcd(time->hours)))
        return false;
...
{
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_WEEKDAY, date->weekday))
        return false;
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_DAY, ds3231_modded_dec_to_bcd(date->day)))
        return false;
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_MONTH, ds3231_modded_dec_to_bcd(date->month)))
        return false;
    if (!ds3231_modded_write_reg(device, DS3231_MODDED_REG_YEAR, ds3231_modded_dec_to_bcd(date->year)))
        return false;
...
bool ds3231_modded_read_temperature(ds3231_modded_t *device, float *temperature)
{
    ...
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_TEMP_MSB, &temp_msb))
        return false;
    if (!ds3231_modded_read_reg(device, DS3231_MODDED_REG_TEMP_LSB, &temp_lsb))
        return false;
